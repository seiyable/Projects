//Brick Game!! ver 1.2 
//can be played with a variable resister through Arduino
...
Serial myPort;
int slideX = 0; //store the value received from Arduino
...
void setup() {
  ...
  //set port number and open Serial port
  myPort = new Serial(this, Serial.list()[8], 9600);
  // will keep receiving serial messages until getting '\n'
  myPort.bufferUntil('\n');
}
...
void serialEvent(Serial myPort) { 
  ...
  int slider = int(myString);

  slideX = int(map(slider, 128, 255, 0, width));
  myPort.write("A");
}
